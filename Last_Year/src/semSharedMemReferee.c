...
static void waitForTeams ()
{
    ...
    /* Bloqueia o fluxo aqui até que a primeira equipa se registe. */
    if (semDown (semgid, sh->refereeWaitTeams) == -1) {                                                      
        perror ("error on the down operation for semaphore (refereeWaitTeams) access (RF)");
        exit (EXIT_FAILURE);
    }

    /* Bloqueia o fluxo aqui até que a segunda equipa se registe. */
    if (semDown (semgid, sh->refereeWaitTeams) == -1) {                                                      
        perror ("error on the down operation for semaphore (refereeWaitTeams) access (RF)");
        exit (EXIT_FAILURE);
    }
...
static void startGame ()
{
    ...
    /* Desbloqueia todos os jogadores que estão à espera para começar a jogar. */
    for (int i = 0; i < NUMPLAYERS; i++) {
        if (semUp (semgid, sh->playersWaitReferee) == -1) {
            perror ("error on the up operation for semaphore (playersWaitReferee) access (RF)");
            exit (EXIT_FAILURE);
        }
    }
...
static void endGame ()
{
    ...
    /* 
    * Desbloqueia todos os jogadores à espera do final do jogo para que cada um deles possa 
    * terminar.
    */
    for (int i = 0; i < NUMPLAYERS; i++) {
        if (semUp (semgid, sh->playersWaitEnd) == -1) {                                                      /* enter critical region */
            perror ("error on the up operation for semaphore (playersWaitEnd) access (RF)");
            exit (EXIT_FAILURE);
        }
    }
