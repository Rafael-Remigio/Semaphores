...
static int goalieConstituteTeam (int id)
{
    ...
    /* Já chegaram todos os guarda-redes necessários, então este guarda redes está atrasado*/
    if (sh->fSt.goaliesArrived > NUMTEAMGOALIES*2) {  
        ...
        /*
        * Não é possível formar a equipa. 
        * Não existem jogadores/guarda-redes livres em quantidade suficiente
        */
        ...
        for (int i = 0; i < NUMTEAMPLAYERS; i++) {
            ...
            /* Desbloquear todos os jogadores à espera de formar equipa. */
            if (semUp (semgid, sh->playersWaitTeam) == -1) {                                                    
                perror ("error on the up operation for semaphore access (GL)");
                exit (EXIT_FAILURE);
            }

            /* Fazer a confirmação de registo na equipa para cada jogador. */
        ...
        /* Dar a conhecer ao árbitro que uma equipa ficou pronta. */
    ...
    /* 
    * Se o guarda-redes não pôde formar uma equipa por falta de jogadores, então ele deve 
    * bloquear aqui e esperar até que "uma equipa o chame".
    * 
    ...
    */
    if (canConstituteTeam == 0) {
        ...
        /* 
        * Quando uma equipa chama o guarda-redes este dá a conhecer a sua entrada na equipa
        * incrementando o semáforo "playerRegistered"
        */
...
static void waitReferee (int id, int team)
{
    ...
    /* Bloqueia o fluxo aqui para esperar que o árbitro dê início à partida. (Incremente o semáforo)*/
...
static void playUntilEnd (int id, int team)
{
    ...
    /* Bloqueia o processo aqui até que o árbitro decida o fim da partida. (Incremente o semáforo). */
